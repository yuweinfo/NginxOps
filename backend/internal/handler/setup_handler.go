...
func (h *SetupHandler) InitializeSystem(c *gin.Context) {
	...
	response.SuccessWithMessage(c, "系统初始化成功", gin.H{
		"success": true,
		"message": "配置已保存，正在启动服务...",
	})

	// 异步启动 PostgreSQL 和初始化数据库
	go func() {
		time.Sleep(500 * time.Millisecond) // 等待响应发送完成
		...
		if !req.UseExternalDB {
			...
			if _, err := exec.Command("test", "-f", initFlag).CombinedOutput(); err != nil {
				...
				if output, err := cmd.CombinedOutput(); err != nil {
					...
					log.Printf("Failed to initialize database cluster: %v", err)
					return
				}
			...
			for i := 0; i < 30; i++ {
				...
				if i == 29 {
					log.Println("PostgreSQL failed to start within 30 seconds")
					return
				}
			}
		...
		log.Println("System is now ready to accept login requests.")
	}()
}
